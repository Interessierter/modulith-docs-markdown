 ...
 * <p>
 * Node definitions look like {@code 3["<div style='font-weight: bold'>Inventory</div>...` and edges look like
 * {@code 4-- "<div>uses</div>..." -->3} (solid) or {@code 4-. "..." .->3} (dashed) — see
 * {@code MermaidDiagramExporter.writeElement/writeRelationship}.
 */
record MermaidGraph(Map<String, String> nodeLabels, Set<String> edgesByLabel) {
	...
	private static final Pattern NODE = Pattern.compile(
			"^(\\S+)\\[\"<div style='font-weight: bold'>(.*?)</div>");
